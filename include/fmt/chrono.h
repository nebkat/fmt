...
// Locale-dependent time formatting. A null locale_ref denotes the classic
// locale. Without locale support std::locale is never instantiated.
#if FMT_USE_LOCALE
...
template <typename Char, typename OutputIt>
auto write_localized(OutputIt out, const std::tm& time, locale_ref loc,
...
auto write_localized_str(OutputIt out, string_view sv, locale_ref loc)
    -> OutputIt {
  return write_tm_str<Char>(
      out, sv, loc ? loc.get<std::locale>() : get_classic_locale());
}
#else
...
template <typename Char, typename OutputIt>
auto write_localized(OutputIt out, const std::tm&, locale_ref, char, char)
    -> OutputIt {
  return out;
}
